<template>
<ion-item-sliding>
    <ion-item button lines="none" class="transaction-item" :class="transaction.amount > 0 ? 'income-border' : 'expense-border'" >
        <ion-label @click="handleEdit">
            <h2>{{ transaction.text }}</h2>
            <p>{{ new Date().toLocaleDateString() }}</p>
        </ion-label>
      <span class="text-center">
        <ion-note :class="transaction.amount > 0 ? 'income-text' : 'expense-text'" @click="handleEdit">
            {{ transaction.amount > 0 ? '+' : '' }}${{ Math.abs(transaction.amount).toFixed(2) }}
        </ion-note>
        <ion-note >
            <ion-item-option color=" " @click="confirmDelete">
            ...
            <ion-card-content>
                <div class="form-group">
                    <ion-item class="custom-input">
                        ...
                        <ion-input v-model="editedTransaction.text" placeholder="What's this transaction?" class="input-field">
                        </ion-input>
                    </ion-item>
                </div>
                ...
                <div class="form-group">
                    <ion-item class="custom-input">
                        ...
                        <ion-input v-model="editedTransaction.amount" type="number" placeholder="Enter amount (- for expense)" class="input-field" :class="editedTransaction.amount && editedTransaction.amount > 0 ? 'income-input' : editedTransaction.amount < 0 ? 'expense-input' : ''">
                        </ion-input>
                    </ion-item>
                </div>
...
<!-- Delete Alert -->
<ion-alert :is-open="isDeleteAlertOpen" header="Confirm Delete" message="Are you sure you want to delete this transaction?" :buttons="[
      {
        text: 'Cancel',
        role: 'cancel',
      },
